...
*** Focus Mode - More Low Distraction Editing.

When faced with a wall of text, it can become a challenge to focus on
what it is that you are trying to write. The Focus Mode configuration
dims any text outside the current working block of text, with the
current block of text being defined depending on the mode that the
text is in. In text and LaTeX the focus is paragraph.
...
#+begin_src emacs-lisp
  ...
  (use-package focus
    ...
    :config (add-to-list 'focus-mode-to-thing '(prog-mode . function-body))
...
This appears to work as I would want it to, at least, in Org
Mode. Testing in LaTeX and programming modes will have to happen
later.
