...
#+begin_src emacs-lisp
    ;; Activate and configure eBib for BibLaTeX database management.
    (use-package ebib
      :ensure ebib
...
Using C-c b to start ebib. The format now uses BibLaTeX, which
according to the documents is a more modern implementation of BibTeX.

More notes should be added in time on the process of adding entries,
as well as time spent actually adding entries.

Then a method for printing out the entries in the bib files needs to
be set up, ideally to print on postcards sized sheets.
...
*** Remember - Emacs Note Taking Facility.

Remember mode is a quick and easy way of creating notes that can be
revisited and processed later.

It may be superseded by the multi-node Roam system where nodes can be
interconnected.
